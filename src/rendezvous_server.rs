...
impl RendezvousServer {
    ...
    pub async fn start_with_bind(
        ...
        if must_login.to_uppercase() == "Y"
            || (must_login.is_empty() && get_arg("MUST_LOGIN").to_uppercase() == "Y")
        {
            ...
            match msg_in.union {
                ...
                Some(rendezvous_message::Union::KeyExchange(ex)) => {
                    ...
                    let their_pk: [u8; 32] = match ex.keys[0].to_vec().try_into() {
                        Ok(v) => v,
                        Err(_) => {
                            log::error!("Handshake failed: invalid public key in key exchange");
                            return false;
                        }
                    };
                    let cryptobox: [u8; 48] = match ex.keys[1].to_vec().try_into() {
                        Ok(v) => v,
                        Err(_) => {
                            log::error!("Handshake failed: invalid sealed box in key exchange");
                            return false;
                        }
                    };
                    let symetric_key = match get_symetric_key_from_msg(
                        ...
                        Some(key) => key,
                        None => {
                            log::error!("Handshake failed: cannot open sealed key from {:?}", addr);
                            return false;
                        }
                    };
                    match secretbox::Key::from_slice(&symetric_key) {
                        Some(key) => {
                            ...
                            log::debug!("KeyExchange symetric key set for {:?}", addr);
                            return true;
                        }
                        None => {
        ...
        match sk {
            Some(sk) => {
                ...
                log::trace!(
                    "KeyExchange {:?} -> bytes: {:?}",
                    addr,
                    hex::encode(Bytes::from(msg_out.write_to_bytes().unwrap_or_default()))
                );
                Self::send_to_sink(sink, msg_out).await;
            }
            None => {}
